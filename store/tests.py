...
def fake_dataset():
    u = User.objects.create_user(
        username="fake@mail.com", password="password", first_name="name"
    )
    fake_adress = Adress.objects.create(
        numero=1, rue="street",
        complement="cplt",
        code_postal=95000,
        ville="city"
    )
    ClientType.objects.create(type_client="particulier")
    ...
    fake_product = Product.objects.create(
        name="fake_product", fk_category=fake_category
    )
    fake_variety = Variety.objects.create(
        name="fake_name",
        price=1,
        stock=20,
        fk_unity=fake_unity,
        fk_product=fake_product,
        image="medias_variety/batavia.jpg",
    )
...
class CartTestCase(TestCase):
    ...
    def test_authenticated_post_remove(self):
        ...
        c.login(username="fake@mail.com", password="password")
        Cart.objects.create(
            fk_client=fake_client, fk_variety=fake_variety, quantity=5
        )
        initial = Cart.objects.count()
        response = c.post(
            reverse("store:cart"),
            {"action": "remove", "cart_object": fake_variety.pk}
        )
    ...
    def test_authenticated_post_update(self):
        ...
        c = C()
        c.login(username="fake@mail.com", password="password")
        initial_cart = Cart.objects.create(
            fk_client=fake_client, fk_variety=fake_variety, quantity=5
        )
        response = c.post(
            reverse("store:cart"),
        ...
        new_cart = Cart.objects.get(
            fk_client=fake_client,
...
class AddCart(TestCase):
    ...
    def test_authenticated_user(self):
        ...
        response = c.post(
            reverse("store:adding_cart"),
            {"product": id_product, "quantity": 2}
        )
...
class AFarmdataTestCase(TestCase):
    ...
    def test_insert_data(self):
        ...
        self.assertEqual(
            len(CommandType.objects.all()), len(fake_database.command_type)
        )
        self.assertEqual(
            len(ClientType.objects.all()),
            len(fake_database.client_type)
        )
        ...
        self.assertEqual(
            len(CollectLocation.objects.all()),
            len(fake_database.emplacement_retrait)
        )
        self.assertEqual(len(Locker.objects.all()), len(fake_database.locker))
        self.assertEqual(
            len(DirectWithdrawal.objects.all()), len(fake_database.withdrawal)
        )
...
class EmailTestCase(TestCase):
    def test_send_email(self):
        User.objects.create(
            username="admin@farm", email="admin@mail.com", password="password"
        )
        c = C()
        response = c.post(
...
class ReservationTestCase(TestCase):
    def test_reservation(self):
        ...
        c.login(username="fake@mail.com", password="password")
        Cart.objects.create(
            fk_client=fake_client, fk_variety=fake_variety, quantity=15
        )
...
class RemoveOrderTestCase(TestCase):
    def test_remove_order(self):
        ...
        fake_order = Order.objects.create(
            fk_client=fake_client, fk_direct_withdrawal=fake_withdrawal
        )
        OrderDetail.objects.create(
            fk_order=fake_order, fk_variety=fake_variety, quantity=15
        )
        OrderHistoric.objects.create(
            fk_order=fake_order, date_creation=datetime.datetime.now()
        )
...
class CustomerJourneyOrderTestCase(StaticLiveServerTestCase):
    ...
    def test_order_process(self):
        ...
        account_url = self.live_server_url + reverse(
            "store:account", kwargs={"info": "success"}
        )
...
class CustomerAccountTestCase(StaticLiveServerTestCase):
    ...
    def test_update(self):
        ...
        name = self.driver.find_element_by_css_selector(
            '#update_infos_form input[name="name"]'
        )
        name.clear()
        name.send_keys("New name")
        self.driver.find_element_by_css_selector(
            '#update_infos_form input[name="password"]'
        ).send_keys("password")
        self.driver.find_element_by_css_selector(
            "#update_infos_form button"
        ).click()
